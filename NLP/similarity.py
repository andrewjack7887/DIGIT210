...
# this all succeeds if you see the text of your files printed in the console.
        ...
        for token in tokens:
            if(token and token.vector_norm):
                ...
                if wordOfInterest.similarity(token) > .55:
        ...
        # print(len(deduped), '\n **** \n', f'{deduped=}')
        # print(len(deduped.items()), "\n vs \n", len(highSimilarityDict.items()))
        ...
        sorted_dict_by_score = dict(sorted(deduped.items(), key=lambda x:x[1]))
        print(f"\n\nThis is the sorted similarity dict (for {wordOfInterest}): \n{sorted_dict_by_score}")
...
readTextFiles("ebb-chatgpt-results.txt")
